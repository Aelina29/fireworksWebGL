function Spark() {
    this.init();
}

function Tail() {
    this.init();
}

// количество искр
Spark.sparksCount = 60;
Tail.tailCount = 60;

Spark.prototype.init = function() {
    // время создания искры
    this.timeFromCreation = performance.now();

    // задаём направление полёта искры в градусах, от 0 до 360
    const angle = Math.random() * 360;
    //const angle = 0.1*360;
    // радиус - это расстояние, которое пролетит искра
    //const radius = Math.random();
    const maxrad = 1.3;
    const minrad = 0.3;
    const radius = Math.random() * (maxrad - minrad) + minrad;
    // отмеряем точки на окружности - максимальные координаты искры
    this.xMax = Math.cos(angle) * radius;
    this.yMax = Math.sin(angle) * radius;

    // dx и dy - приращение искры за вызов отрисовки, то есть её скорость,
    // у каждой искры своя скорость. multiplier подобран экспериментально
    //const multiplier = 125 + Math.random() * 125;
    const multiplier = 4000;
    this.dx = -this.xMax / multiplier;
    this.dy = -this.yMax / multiplier;

    //прозрачность искры
    this.trasparence = 1.1;
    this.delta_trasparence = radius / 250;

    //+смещаем финальную точку
    this.xMax += kx;
    this.yMax += ky;

    // Для того, чтобы не все искры начинали движение из начала координат,
    // делаем каждой искре свой отступ, но не более максимальных значений.
    //+смещаем начальную точку
    this.x = kx + (this.dx * 1000) % this.xMax;
    this.y = ky + (this.dy * 1000) % this.yMax;

    // const maxsize = 20.0;
    // const minsize = 50.0;
    // this.size_of_point = Math.random() * (maxsize - minsize) + minsize;
    this.size_of_point = 50.0;
};

Spark.prototype.move = function(time) {
    // находим разницу между вызовами отрисовки, чтобы анимация работала
    // одинаково на компьютерах разной мощности
    const timeShift = time - this.timeFromCreation;
    this.timeFromCreation = time;

    // приращение зависит от времени между отрисовками
    const speed = timeShift;
    this.x += this.dx * speed;
    this.y += this.dy * speed;
    this.trasparence -=  this.delta_trasparence;

    // если искра достигла конечной точки, запускаем её заново из начала координат
    if (Math.abs(this.x) > Math.abs(this.xMax) || Math.abs(this.y) > Math.abs(this.yMax)) {
        this.init();
    }
};

Tail.prototype.init = function() {
    // время создания искры
    this.timeFromCreation = performance.now();

    // задаём направление полёта искры в градусах, от 0 до 360
    //const angle = Math.random() * 360;
    // радиус - это расстояние, которое пролетит искра
    //const radius = Math.random();
    const maxrad = 2.0;
    const minrad = 0.3;
    const radius = Math.random() * (maxrad - minrad) + minrad  + 2;
    // отмеряем точки на окружности - максимальные координаты искры
    this.yMax = radius - 2;

    // dx и dy - приращение искры за вызов отрисовки, то есть её скорость,
    // у каждой искры своя скорость. multiplier подобран экспериментально
    //const multiplier = 125 + Math.random() * 125;
    const multiplier = 8000;
    this.dy = radius / multiplier;

    //прозрачность искры
    this.trasparence = 1.1;
    this.delta_trasparence = radius / 250;

    //+смещаем финальную точку
    //this.yMax += ky;

    // Для того, чтобы не все искры начинали движение из начала координат,
    // делаем каждой искре свой отступ, но не более максимальных значений.
    //+смещаем начальную точку
    this.y = -2;
    this.x = Math.random() * (-4 - 4) + 4;  
};

Tail.prototype.move = function(time) {
    // находим разницу между вызовами отрисовки, чтобы анимация работала
    // одинаково на компьютерах разной мощности
    const timeShift = time - this.timeFromCreation;
    this.timeFromCreation = time;

    // приращение зависит от времени между отрисовками
    const speed = timeShift;
    this.y += this.dy * speed;
    this.trasparence =  this.delta_trasparence;

    // если искра достигла конечной точки, запускаем её заново из начала координат
    if (Math.abs(this.y) > Math.abs(this.yMax)) {
        this.init();
    }
};


function main() {
    const canvas = document.getElementById("my_canvas");
    gl = canvas.getContext("webgl");    
    if (!gl) {
        exit;
    }
    hei = gl.canvas.height;
    wid = gl.canvas.width;
    let tails = [];
    for (let i = 0; i < Tail.tailCount; i++) {
        tails.push(new Tail());
    }
    let sparks = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks.push(new Spark());
    }
    let sparks1 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks1.push(new Spark());
    }
    let sparks2 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks2.push(new Spark());
    }
    let sparks3 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks3.push(new Spark());
    }
    let sparks4 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks4.push(new Spark());
    }
    let sparks5 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks5.push(new Spark());
    }
    let sparks6 = [];
    for (let i = 0; i < Spark.sparksCount; i++) {
        sparks6.push(new Spark());
    }
    // gl.SRC_ALPHA - рисуемая искра умножается на прозрачный канал, чтобы убрать фон
    // изображения. gl.ONE - уже нарисованные искры остаются без изменений
    gl.enable(gl.BLEND);//Смешивание искр
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE);

    let programTrack = webglUtils.createProgramFromScripts(gl, ["vertex-shader-track", "fragment-shader-track"]);
    let positionAttributeLocationTrack = gl.getAttribLocation(programTrack, "a_position");
    let colorAttributeLocationTrack = gl.getAttribLocation(programTrack, "a_color");
    let pMatrixUniformLocationTrack = gl.getUniformLocation(programTrack, "u_pMatrix");
    let mvMatrixUniformLocationTrack = gl.getUniformLocation(programTrack, "u_mvMatrix");

    let programSpark = webglUtils.createProgramFromScripts(gl, ["vertex-shader-spark", "fragment-shader-spark"]);
    let positionAttributeLocationSpark = gl.getAttribLocation(programSpark, "a_position");
    let positionAttributeLocationSparkSize = gl.getAttribLocation(programSpark, "a_size");
    let positionAttributeLocationSparkTranspar = gl.getAttribLocation(programSpark, "a_trasparence");
    let textureLocationSpark = gl.getUniformLocation(programSpark, "u_texture");
    let pMatrixUniformLocationSpark = gl.getUniformLocation(programSpark, "u_pMatrix");
    let mvMatrixUniformLocationSpark = gl.getUniformLocation(programSpark, "u_mvMatrix");

    let programTail = webglUtils.createProgramFromScripts(gl, ["vertex-shader-track", "fragment-shader-track"]);
    let positionAttributeLocationTail = gl.getAttribLocation(programTail, "a_position");
    let colorAttributeLocationTail = gl.getAttribLocation(programTail, "a_color");
    let positionAttributeLocationSparkTail = gl.getAttribLocation(programSpark, "a_trasparence");
    let pMatrixUniformLocationTail = gl.getUniformLocation(programTail, "u_pMatrix");
    let mvMatrixUniformLocationTail = gl.getUniformLocation(programTail, "u_mvMatrix");

    let texture = gl.createTexture();
    let image = new Image();
    image.src = "fireworks/pngwing.com(1).png";
    let loaded = false;

    let texture1 = gl.createTexture();
    let image1 = new Image();
    image1.src = "fireworks/pngwing.com(8).png";
    let loaded1 = false;

    let texture2 = gl.createTexture();
    let image2 = new Image();
    image2.src = "fireworks/pngwing.com(9).png";
    let loaded2 = false;
    
    // let texture3 = gl.createTexture();
    // let image3 = new Image();
    // image3.src = "fireworks/pngwing.com(7).png";
    // let loaded3 = false;

    image.addEventListener('load', function() {
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);
        gl.generateMipmap(gl.TEXTURE_2D);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        // Не допускаем повторения по s-координате.
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
        gl.bindTexture(gl.TEXTURE_2D, null);
        loaded = true;
    });
    image1.addEventListener('load', function() {
        gl.bindTexture(gl.TEXTURE_2D, texture1);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image1);
        gl.generateMipmap(gl.TEXTURE_2D);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        // Не допускаем повторения по s-координате.
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
        gl.bindTexture(gl.TEXTURE_2D, null);
        loaded1 = true; 
    });
    image2.addEventListener('load', function() {
        gl.bindTexture(gl.TEXTURE_2D, texture2);
        gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image2);
        gl.generateMipmap(gl.TEXTURE_2D);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
        // Не допускаем повторения по s-координате.
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
        gl.bindTexture(gl.TEXTURE_2D, null);
        loaded2 = true; 
    });
    // image3.addEventListener('load', function() {
    //     gl.bindTexture(gl.TEXTURE_2D, texture3);
    //     gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image3);
    //     gl.generateMipmap(gl.TEXTURE_2D);
    //     gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    //     gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    //     // Не допускаем повторения по s-координате.
    //     gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    //     gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    //     gl.bindTexture(gl.TEXTURE_2D, null);
    //     loaded3 = true; 
    // });
    requestAnimationFrame(drawScene);

    let mvMatrix = mat4.create();
    let pMatrix = mat4.create();

    function drawTracks(positions,center) {
        let colors = [];
        let positionsFromCenter = [];
        for (let i = 0; i < positions.length; i += 3) {
            // для каждой координаты добавляем точку начала координат, чтобы получить след искры
            positionsFromCenter.push(center[0],center[1],center[2]);
            //positionsFromCenter.push(0,0,0)
            positionsFromCenter.push(positions[i], positions[i + 1], positions[i + 2]);

            // цвет в начале координат будет белый (горячий), а дальше будет приближаться к оранжевому
            //colors.push(1, 1, 1, 0.47, 0.31, 0.24);
            //colors.push(1,1,1,0,0,0);
            colors.push(0,0,0,1,1,1);
        }

        gl.useProgram(programTrack);

        gl.uniformMatrix4fv(pMatrixUniformLocationTrack, false, pMatrix);
        gl.uniformMatrix4fv(mvMatrixUniformLocationTrack, false, mvMatrix);

        let positionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(positionsFromCenter), gl.STATIC_DRAW);
        gl.vertexAttribPointer(positionAttributeLocationTrack, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttributeLocationTrack);

        let colorBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);
        gl.vertexAttribPointer(colorAttributeLocationTrack, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(colorAttributeLocationTrack);

        gl.drawArrays(gl.LINES, 0, positionsFromCenter.length / 3);
    }

    function drawTracksUp(tails_positions) {
        let colors = [];
        let positionsFromCenter = [];
        for (let i = 0; i < tails_positions.length; i+=2) {
            // для каждой координаты добавляем точку начала координат, чтобы получить след искры
            //const x = Math.random() * (-4 - 4) + 4;  
            //const y = Math.random() * (-1 - 1.3) + 1.3; 
            const y = tails_positions[i+1]    
            const x =  tails_positions[i]       
            positionsFromCenter.push(x, -2, 0);
            positionsFromCenter.push(x, y, 0);

            // цвет в начале координат будет белый (горячий), а дальше будет приближаться к оранжевому
            //colors.push(1,1,1,0,0,0);
            colors.push(0,0,0,1,1,1);
        }
        //console.log(positionsFromCenter.length);
        gl.useProgram(programTrack);

        gl.uniformMatrix4fv(pMatrixUniformLocationTrack, false, pMatrix);
        gl.uniformMatrix4fv(mvMatrixUniformLocationTrack, false, mvMatrix);

        let positionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(positionsFromCenter), gl.STATIC_DRAW);
        gl.vertexAttribPointer(positionAttributeLocationTrack, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttributeLocationTrack);

        let colorBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, colorBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(colors), gl.STATIC_DRAW);
        gl.vertexAttribPointer(colorAttributeLocationTrack, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(colorAttributeLocationTrack);

        gl.drawArrays(gl.LINES, 0, positionsFromCenter.length / 3);
    }

    function drawSparks(positions, sizes_of_points,trasparence,texture) {
        gl.useProgram(programSpark);

        gl.uniformMatrix4fv(pMatrixUniformLocationSpark, false, pMatrix);
        gl.uniformMatrix4fv(mvMatrixUniformLocationSpark, false, mvMatrix);

        let positionBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(positions), gl.STATIC_DRAW);

        let sizesBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, sizesBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(sizes_of_points), gl.STATIC_DRAW);

        
        let transpBuffer = gl.createBuffer();
        gl.bindBuffer(gl.ARRAY_BUFFER, transpBuffer);
        gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(trasparence), gl.STATIC_DRAW);

        gl.activeTexture(gl.TEXTURE0);
        gl.bindTexture(gl.TEXTURE_2D, texture);
        gl.uniform1i(textureLocationSpark, 0);

        gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer);  
        gl.vertexAttribPointer(positionAttributeLocationSpark, 3, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttributeLocationSpark);
              
        gl.bindBuffer(gl.ARRAY_BUFFER, sizesBuffer);
        gl.vertexAttribPointer(positionAttributeLocationSparkSize, 1, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttributeLocationSparkSize);

        gl.bindBuffer(gl.ARRAY_BUFFER, transpBuffer);
        gl.vertexAttribPointer(positionAttributeLocationSparkTranspar, 1, gl.FLOAT, false, 0, 0);
        gl.enableVertexAttribArray(positionAttributeLocationSparkTranspar);

        gl.drawArrays(gl.POINTS, 0, positions.length / 3);
    }

    function drawScene(now) {
        if(loaded2 && loaded1 && loaded){//чтоб рисовал только после полной загрузки картинки
        // обновляем размер canvas на случай, если он растянулся или сжался вслед за страницей
        webglUtils.resizeCanvasToDisplaySize(gl.canvas);
        gl.viewport(0, 0, gl.canvas.width, gl.canvas.height);
        gl.clearColor(0, 0, 0, 1);
        gl.clear(gl.COLOR_BUFFER_BIT | gl.DEPTH_BUFFER_BIT);
        mat4.perspective(pMatrix, 45, gl.canvas.width / gl.canvas.height, 0.1, 100.0);
        mat4.identity(mvMatrix);
        mat4.translate(mvMatrix, mvMatrix, [0, 0, -3.5]);//смещать взрыв тут
        
        for (let i = 0; i < tails.length; i++) {
            tails[i].move(now);
        }
             
        for (let i = 0; i < sparks6.length; i++) {
            sparks6[i].move(now);
        }
        for (let i = 0; i < sparks2.length; i++) {
            sparks2[i].move(now);
        }

        if(Math.floor(performance.now()/1000) > 7){
            for (let i = 0; i < sparks1.length; i++) {
                sparks1[i].move(now);
            }
            for (let i = 0; i < sparks4.length; i++) {
                sparks4[i].move(now);
            }
            for (let i = 0; i < sparks5.length; i++) {
                sparks5[i].move(now);
            }
        }

        if(Math.floor(performance.now()/1000) > 3){
            for (let i = 0; i < sparks.length; i++) {
                sparks[i].move(now);
            }
            for (let i = 0; i < sparks3.length; i++) {
                sparks3[i].move(now);
            }
        }
        
       
        let tails_positions = [];
        tails.forEach(function(item) {
            tails_positions.push(item.x);
            tails_positions.push(item.y);
        });      

        drawTracksUp(tails_positions);

        let sizes_of_points = [];
        sparks.forEach(function(item) {
            sizes_of_points.push(item.size_of_point);
        });

        let trasparence = [];
        sparks.forEach(function(item) {
            trasparence.push(item.trasparence);
            trasparence.push(item.trasparence);
        });

        let track_trasparence = [];
        sparks.forEach(function(item) {
            track_trasparence.push(item.trasparence);
        });
        
        //const radius = Math.random() * (maxrad - minrad) + minrad;
        const x = Math.random() * (-3.5 - 3.5) + 3.5;
        const y = Math.random() * (-1.0 - 1.5) + 1.5;
        let center = [];
        center.push(x,y,0);

        ///=====================================================================

        let timeIntervals = [];
        for (let i = 0; i < 3; i++) {
            timeIntervals.push(2);
        }

        //number nowt = performance.now()/10;
        console.log(Math.floor(performance.now()/1000));


        let positions6 = [];
        sparks6.forEach(function(item) {
            positions6.push(item.x+2.7);
            positions6.push(item.y-1.2);
            positions6.push(0);
        });
        drawTracks(positions6, [2.7,-1.2,0]);
        drawSparks(positions6, sizes_of_points, trasparence,texture);

        let positions2 = [];
        sparks2.forEach(function(item) {
            positions2.push(item.x-1);
            positions2.push(item.y-1.1);
            positions2.push(0);
        });
        drawTracks(positions2, [-1,-1.1,0]);
        drawSparks(positions2, sizes_of_points, trasparence,texture1);

        if(Math.floor(performance.now()/1000) > 7){
            let positions1 = [];
            sparks1.forEach(function(item) {
                positions1.push(item.x);
                positions1.push(item.y);
                positions1.push(0);
            });
            drawTracks(positions1, [0,0,0]);
            drawSparks(positions1, sizes_of_points, trasparence,texture2);//2);

            let positions4 = [];
            sparks4.forEach(function(item) {
                positions4.push(item.x+3);
                positions4.push(item.y+1);
                positions4.push(0);
            });
            drawTracks(positions4, [3,1,0]);
            drawSparks(positions4, sizes_of_points, trasparence,texture1);   

            let positions5 = [];
            sparks5.forEach(function(item) {
                positions5.push(item.x-1.7);
                positions5.push(item.y+0.5);
                positions5.push(0);
            });
            drawTracks(positions5, [-1.7,0.5,0]);
            drawSparks(positions5, sizes_of_points, trasparence,texture);//3);
        }      

        if(Math.floor(performance.now()/1000) > 3){
            let positions = [];
            sparks.forEach(function(item) {
                positions.push(item.x+1);
                positions.push(item.y+1);
                positions.push(0);
            });
            drawTracks(positions, [1,1,0]);
            drawSparks(positions, sizes_of_points, trasparence,texture1);
            
            let positions3 = [];
            sparks3.forEach(function(item) {
                positions3.push(item.x-3);
                positions3.push(item.y+1.5);
                positions3.push(0);
            });
            drawTracks(positions3, [-3,1.5,0]);
            drawSparks(positions3, sizes_of_points, trasparence,texture2);//2);
        }
        
        
        // let positions2 = [];
        // sparks.forEach(function(item) {
        //     positions2.push(item.x-1);
        //     positions2.push(item.y+1);
        //     positions2.push(0);
        // });
        //drawTracks(positions2, [-1,1,0]);
        //drawSparks(positions2, sizes_of_points, trasparence);   
        
        ///=====================================================================
        

        requestAnimationFrame(drawScene);
    }}
}

var kx = 0;
var ky = 0;
var hei;
var wid;
main();